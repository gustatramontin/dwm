...
Drw *drw_create(Display *dpy, int screen, Window root, unsigned int w,
  ...
  XSetArcMode(dpy, drw->gc, ArcPieSlice);
...
void drw_arc(Drw *drw, int x, int y, unsigned int w, unsigned int h, int filled,
             int invert) {
  if (!drw || !drw->scheme)
    return;
  XSetForeground(drw->dpy, drw->gc,
                 invert ? drw->scheme[ColBg].pixel : drw->scheme[ColFg].pixel);
  if (filled)
    XFillArc(drw->dpy, drw->drawable, drw->gc, x, y, w - 1, h - 1, 0, 360 * 64);
